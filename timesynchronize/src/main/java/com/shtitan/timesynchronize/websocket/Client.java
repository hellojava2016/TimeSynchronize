...
import com.shtitan.timesynchronize.websocket.message.BinaryWebsocketMessage;
import com.shtitan.timesynchronize.websocket.message.CloseWebsocketMessage;
import com.shtitan.timesynchronize.websocket.message.StringWebsocketMessage;
    ...
    private void internalSendMessageAsync(AbstractWebsocketMessage msg) {
        try {
            if (msg instanceof StringWebsocketMessage) {
                ...
                // Close the session.
                session.close();
            }else if(msg instanceof AlarmWebSocketMessage){
            	AlarmWebSocketMessage sMsg = (AlarmWebSocketMessage) msg;
                 async.sendText(sMsg.toJsonString(), sendHandler);
            }
