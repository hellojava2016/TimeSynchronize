package com.shtitan.timesynchronize.dao;
...
import com.shtitan.timesynchronize.util.PropertyFilter;
import com.shtitan.timesynchronize.util.ReflectionUtils;
...
public class HibernateDao<T, PK extends Serializable> {
	...
	public void deleteEntityByProperty(String propertyName, final Object value) {
		getSession().createQuery(
				"delete from " + entityClass.getName() + " where "
						+ propertyName + "='" + value.toString()+"'").executeUpdate();
	}
	 ...
	 *            命名参数,按名称绑定.
	 */
	public Query createQuery(final Map<String, ?> values,final String queryString) {
		Assert.hasText(queryString, "queryString不能为空");
		Query query = getSession().createQuery(queryString);
		if (values != null) {
	...
	public Page<T> findPage(final String hql,final Page<T> page) {
	...
	public T getUniqueEntityByPropNames(final String[] propertyNames,
			final Object[] values) {
		if (ArrayUtils.isEmpty(propertyNames) || ArrayUtils.isEmpty(values)
				|| propertyNames.length != values.length) {
			throw new IllegalArgumentException("arguments is invalid.");
