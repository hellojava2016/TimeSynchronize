		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>主机</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/host/query.do">
								<div class="form-group">
									<div class="col-xs-12 row">
										<div class="col-xs-3 pr_0 pl_0 mr_5 wid_30">
											<span>金融机构</span> 
											<select name="Organization" id="Organization" class="selectcsss">											    
										...
										<div class="col-xs-2 pr_0 pl_0 wid_23">
											<span>设备名称：</span> <input type="text"
												class="form-control ml_5 wid_65"  id="name" name="name"
												value="${name}">
										</div>
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												...
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/host/update_pre.do?id=${rate.serverId}">修改 </a></c:if>
												<a href="${ctx}/asset/topo/pc.do?pcid=${rate.serverId}">查看拓扑</a>
												</td> 
											</tr>
										</c:forEach>
											</c:when>
										<c:otherwise>
											<tr>
												<td colspan="13" class="emptyResut">暂无数据</td>
											</tr>
										</c:otherwise>
									</c:choose>
									<tr style="background-color:#eee">
											<th colspan="13"><tags:pagination page="${rates}" paginationSize="5" />	</th>
										</tr>
									</tbody>
								</table>
        </div></div>
	...
	var pageNo =${pageNo};
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/host/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		}
		//全选的动作
		...
		function newreport(){
			window.location.href="${ctx}/asset/host/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/host/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/host/export.do?"+getQueryParam();
		}
