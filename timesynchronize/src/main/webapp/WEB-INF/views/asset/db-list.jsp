		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>数据库</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/db/query.do">
								<div class="form-group">
									<div class="col-xs-12 row">
										...
										<div class="col-xs-2 pr_0 pl_0 wid_23">
											<span>数据库名称：</span> <input type="text"
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												...
												<td>${rate.databaseName}</td>
												<td>${rate.osinfo.name}</td>
												<td>${rate.type}</td>
												<td>${rate.version}</td>
												<td>${rate.patch}</td>
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/db/update_pre.do?id=${rate.dbId}">修改 </a></c:if>
											</tr>
										</c:forEach>
											</c:when>
										<c:otherwise>
	...
	var pageNo =${pageNo};
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/db/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		...
		function newreport(){
			window.location.href="${ctx}/asset/db/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/db/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/db/export.do?"+getQueryParam();
		}
