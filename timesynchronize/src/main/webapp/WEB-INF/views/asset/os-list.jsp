		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>操作系统</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/os/query.do">
								<div class="form-group">
									<div class="col-xs-12 row">
										...
										<div class="col-xs-3 pr_0 pl_0 wid_30">
											<span>操作系统名称：</span> <input type="text"
								...
								<table id="contentTable" class="table table-striped table-bordered table-condensed">
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												<td><input type="checkbox" name="rolecheckbox" value="${rate.osId}" /></td>
												<td>${rate.organization.name}</td>
												<td>${rate.name}</td>
												<td>${rate.uniqueVal}</td>
												<td>${rate.pcserver.name}</td>												
												<td>${rate.type}</td>
												<td>${rate.version}</td>
												<td>${rate.patch}</td>
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/os/update_pre.do?id=${rate.osId}">修改 </a></c:if>
											</tr>
										</c:forEach>
										</c:when>
										<c:otherwise>
											<tr>
												<td colspan="9" class="emptyResut">暂无数据</td>
											</tr>
										</c:otherwise>
									</c:choose>
									<tr style="background-color:#eee">
											<th colspan="9"><tags:pagination page="${rates}" paginationSize="5" />	</th>
									</tr>
									</tbody>
								</table>
	...
	var pageNo =${pageNo};
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/os/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		}
		//全选的动作
		...
		function newreport(){
			window.location.href="${ctx}/asset/os/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/os/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/os/export.do?"+getQueryParam();
		}
		...
		function topo(){
			window.open("${ctx}/asset/topo/topo.do");
		}
	</script>
</body>
</html>