		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>机房UPS</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/upsinfo/query.do">
								<div class="form-group">
									<div class="col-xs-12 row">
										...
										<div class="col-xs-2 pr_0 pl_0 wid_23">
											<span>UPS名称：</span> <input type="text"
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												...
												<td>${rate.type}</td>
												<td>${rate.name}</td>
												<td>${rate.inputVoltage}</td>
												<td>${rate.outputVoltage}</td>
												<td>${rate.inputFrequency}</td>
												<td>${rate.outputFrequency}</td>
												<td>${rate.power}</td>
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/upsinfo/update_pre.do?id=${rate.upsId}">修改 </a></c:if> 
											</tr>
										</c:forEach>
										</c:when>
										<c:otherwise>
	...
	var pageNo =${pageNo};
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/upsinfo/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		}
		//全选的动作
		...
		function newreport(){
			window.location.href="${ctx}/asset/upsinfo/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/upsinfo/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/upsinfo/export.do?"+getQueryParam();
		}
