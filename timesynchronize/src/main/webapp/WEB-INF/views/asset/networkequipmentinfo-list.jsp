		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>网络设备</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/networkequipmentinfo/query.do">
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												...
												<td>${rate.purpose}</td>
												<td>${rate.location}</td>
												<td>${rate.portCount}</td>
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/networkequipmentinfo/update_pre.do?id=${rate.neId}">修改 </a></c:if>
											</tr>
										</c:forEach>
										</c:when>
										<c:otherwise>
	...
	var pageNo =${pageNo};
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/networkequipmentinfo/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		}
		//全选的动作
		...
		function newreport(){
			window.location.href="${ctx}/asset/networkequipmentinfo/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/networkequipmentinfo/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/networkequipmentinfo/export.do?"+getQueryParam();
		}
