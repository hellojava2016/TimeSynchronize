		...
		<div id="content" class="row mt_78">
			...
			<div class="col-md-10" id="right">
				<div class="breadcrumb">
					<span>您当前所在的位置：</span> > <a>配置信息</a> > <a>机房</a>
				</div>
				...
				<div class="row whitebackground mb_10">
					<div class="span12">
						...
						<div class="show_conment">
							<form class="form-horizontal" action="${ctx}/asset/computerroominfo/query.do">
									...
									<c:when test="${!empty rates.result }">
										<c:forEach items="${rates.result}" var="rate">
											<tr>
												...
												<td>${rate.location}</td>
												<td>${rate.power}</td>
												<td><fmt:formatDate value="${rate.serverTime}" pattern="yyyy-MM-dd"/></td>
												<td>${rate.upsCount}</td>
												<td>${rate.precisionAcCount}</td>
												<c:set var="permisions" value="${permisions}" scope="session" /> 
												<td><c:if test="${fn:contains(permisions, 'asset:modify')}"><a href="${ctx}/asset/computerroominfo/update_pre.do?id=${rate.crId}">修改 </a></c:if>
											</tr>
										</c:forEach>
										</c:when>
										<c:otherwise>
											<tr>
												<td colspan="12" class="emptyResut">暂无数据</td>
											</tr>
										</c:otherwise>
									</c:choose>
									<tr style="background-color:#eee">
											<th colspan="12"><tags:pagination page="${rates}" paginationSize="5" />	</th>
										</tr>
									</tbody>
								</table>
	...
	var pageNo =${pageNo};
		...
		function deleteRates() {
			...
			$.messager.confirm("系统提示", "您确定删除所选数据？", function() {
				window.location.href  = "${ctx}/asset/computerroominfo/delete.do?pageNo="+pageNo+"&deleteids="+deleteroleids;
			});
		}
		//全选的动作
		...
		function newreport(){
			window.location.href="${ctx}/asset/computerroominfo/add_pre.do";
		}
		function refresh(){
			window.location.href="${ctx}/asset/computerroominfo/query.do?"+getQueryParam();
		}
		function exportSystem(){
			window.location.href="${ctx}/asset/computerroominfo/export.do?"+getQueryParam();
		}
		function getQueryParam(){
			var Organization = $("#Organization").val();
			var uniqueVal = $("#uniqueVal").val();
			return "Organization="+Organization+"&uniqueVal="+uniqueVal+"&pageNo="+pageNo;
		}
	</script>
</body>
</html>