...
$(function (){
	$("#city_5").citySelect({
		url:"<%=request.getContextPath()%>/report/systemavailablerate/gettypes.do",
		prov:"${prov}",
		city:"${city}",
		dist:"${dist}",
		nodata:"none"
...
<div class="container"> 
  ...
  <div id="content" class="row mt_78">
    ...
    <div class="col-md-10" id="right">
      <div class="breadcrumb"><span>您当前所在的位置：</span> <a>风险分析</a> &gt; <a>系统可用率</a></div>
   <div class="row whitebackground mb_10">
               ...
               <div class="col-xs-12 row dpheight15"></div>
                 ...
                 <div class="col-xs-3 pr_0 pl_0 mr_10">  
                  <select name="reportorder"  class="selectcsss" id="reportorder">                      
                      <option value="4" <c:if test="${reportorder==4}">selected</c:if>>系统可用率-降序</option> 
                      <option value="5" <c:if test="${reportorder==5}">selected</c:if>>系统可用率-升序</option>                 
	...
	<div class="pull-right">
		...
		<thead><tr><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>预期停止服务时间(分钟)</th><th>非预期停止服务时间(分钟)</th><th>提供服务总时间(分钟)</th><th>名义可用率(%)</th><th>实际可用率(%)</th></tr></thead>
		<tbody>
		<c:choose>
		...
		<c:forEach items="${rates.result}" var="rate">
			<tr>
				...
				<td>${rate.ud}</td>
				<td>${rate.ltsp}</td>		
				<td><fmt:formatNumber  value="${rate.percentExpected}"   type="number" pattern="0.00" /></td>
				<td><fmt:formatNumber  value="${rate.percentActual}"   type="number" pattern="0.00" />	
			</tr>
		          </c:forEach>
	       	     </c:when>
						<c:otherwise>
		    ...
		    <tr style="background-color:#eee">
					<th colspan="10"><tags:pagination page="${rates}" paginationSize="10"/></th>
			</tr>
		</tbody>
	</table>
...
function refresh(){
	window.location.href="${ctx}/report/systemavailablerate/report.do?"+getQueryParam();
}
</script>
</body>
</html>