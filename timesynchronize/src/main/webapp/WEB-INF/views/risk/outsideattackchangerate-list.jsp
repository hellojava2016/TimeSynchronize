...
$(function (){
	$("#city_5").citySelect({
		url:"<%=request.getContextPath()%>/report/outsideattackchangerate/gettypes.do",
		prov:"${prov}",
		city:"${city}",
		dist:"${dist}",
		nodata:"none"
	...
	var url = '${ctx}/report/outsideattackchangerate/upload.do';
	$('#fileupload').fileupload({
	    ...
	    add: function (e, data) { 
	    	 ...
	    	 window.location.href = "${ctx}/report/outsideattackchangerate/list.do?message="+data.result;
	    }
...
function deleteRates(){
	var deleteroleids = "";
	$("input[name='riskcheckbox']:checked").each(function(){     
		deleteroleids+=$(this).val()+",";     
	    })  ;
	if(""==deleteroleids){
		 $.messager.alert("系统提示", "请选择要删除的指标！");
		 return;
	} 
	$.messager.model = { 
	      ...
	      $.messager.confirm("系统提示", "您确定删除所选指标？", function() { 
	    	    ajaxOptions.url = "${ctx}/report/outsideattackchangerate/delete.do";
	    		ajaxOptions.params = {"pageNo":pageNo,"deleteids":deleteroleids};
	    		ajaxOptions.success = function(msg){
	    			window.location.href="${ctx}/report/outsideattackchangerate/query.do?"+getQueryParam();
	    	    };
...
function selectAll(object){  
    ...
    $("input[name='riskcheckbox']").prop("checked", false);  
    }  
    return false;
} 
...
function newreport(){
	window.location.href="${ctx}/report/outsideattackchangerate/add_pre.do";
}
function refresh(){
	window.location.href="${ctx}/report/outsideattackchangerate/query.do?"+getQueryParam();
}
function exportSystem(){
	window.location.href="${ctx}/report/outsideattackchangerate/export.do?"+getQueryParam();
}
...
<div class="container"> 
  ...
  <div id="content" class="row mt_78">
    ...
    <div class="col-md-10" id="right">
      <div class="breadcrumb"><span>您当前所在的位置：</span> &gt; <a>数据采集</a> &gt; <a>外部攻击变化率</a></div>
        <div class="row whitebackground mb_10">
        <div class="span12">
          <h3 class="show_tit">查询</h3>
          <div class="show_conment">
            <form class="form-horizontal" action="${ctx}/report/outsideattackchangerate/query.do?">          
      ...
      <div class="row show mb_10">
        <div class="span12">
           ...
           <div id="operateBtns">
	<div class="pull-left">
	<h4 class="showtit4">外部攻击变化率列表</h4>
	</div>
<div class="pull-right">
		...
		<thead><tr><th><input type="checkbox" id="allroleselected" onclick="selectAll(this)"/></th><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>入侵检测系统告警数</th><th>入侵保护系统告警数</th></tr></thead>
		<tbody>
		<c:choose>
			 	<c:when test="${!empty rates.result }">
		<c:forEach items="${rates.result}" var="rate">
			<tr>
			    <td><input type="checkbox" name="riskcheckbox" value="${rate.id}" class="ml_11"/></td>
