  ...
  <div class="row whitebackground mb_10">
        <div class="span12">
          ...
          <div class="show_conment">
            <form class="form-horizontal" action="${ctx}/report/infotechnologyriskeventcount/query.do?">          
	...
	<div class="pull-right">
		...
		<thead><tr><th><input type="checkbox" id="allroleselected" onclick="selectAll(this)"/></th><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>风险数量</th></tr></thead>
		<tbody>
		<c:choose>
...
var url = '${ctx}/report/infotechnologyriskeventcount/upload.do';
$('#fileupload').fileupload({
    ...
    add: function (e, data) { 
    	 ...
    	 window.location.href = "${ctx}/report/infotechnologyriskeventcount/list.do?message="+data.result;
    }
	...
	if(""==deleteroleids){
		 ...
		 return;
	      ...
	      $.messager.confirm("系统提示", "您确定删除所选数据？", function() { 
	    	    ajaxOptions.url = "${ctx}/report/infotechnologyriskeventcount/delete.do?";
	    		ajaxOptions.params = {"pageNo":pageNo,"deleteids":deleteroleids};
	    		ajaxOptions.success = function(msg){
	    			window.location.href = "${ctx}/report/infotechnologyriskeventcount/query.do?"+getQueryParam();
	    	    };
...
function newreport(){
	window.location.href="${ctx}/report/infotechnologyriskeventcount/add_pre.do";
}
function refresh(){
	window.location.href="${ctx}/report/infotechnologyriskeventcount/query.do?"+getQueryParam();
}
function exportSystem(){
	window.location.href="${ctx}/report/infotechnologyriskeventcount/export.do?"+getQueryParam();
}
