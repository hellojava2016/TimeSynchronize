...
$(function (){
$("#city_5").citySelect({
	url:"<%=request.getContextPath()%>/report/operationchangesuccessrate/gettypes.do",
	prov:"${prov}",
	city:"${city}",
	dist:"${dist}",
	nodata:"none"
...
var url = '${ctx}/report/operationchangesuccessrate/upload.do';
$('#fileupload').fileupload({
    ...
    add: function (e, data) { 
    	 ...
    	 window.location.href = "${ctx}/report/operationchangesuccessrate/list.do?message="+data.result;
    }
   ...
   <div class="row whitebackground mb_10">
        <div class="span12">
          ...
          <div class="show_conment">
            <form class="form-horizontal" action="${ctx}/report/operationchangesuccessrate/query.do?">          
      ...
      <div class="row whitebackground mb_10">
        <div class="span12">
          <div class="show_tit3">
           <div id="operateBtns">
	<div class="pull-left">
	<h4 class="showtit4">投产变更成功率列表</h4>
	</div>
	<div class="pull-right">
		...
		<thead><tr><th><input type="checkbox" id="alloperselected" onclick="selectAll(this)"/></th><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>总实施数量</th><th>成功实施数量</th></tr></thead>
		<tbody>
		<c:choose>
			 	<c:when test="${!empty rates.result }">
		<c:forEach items="${rates.result}" var="rate">
			<tr>
			 <td><input type="checkbox" name="opercheckbox" value="${rate.id}"/></td>
...
function deleteRates(){
	var deleteroleids = "";
	$("input[name='opercheckbox']:checked").each(function(){     
	...
	if(""==deleteroleids){
		 ...
		 return;
	      ...
	      $.messager.confirm("系统提示", "您确定删除所选数据？", function() { 
	    	    ajaxOptions.url = "${ctx}/report/operationchangesuccessrate/delete.do?";
	    		ajaxOptions.params = {"pageNo":pageNo,"deleteids":deleteroleids};
	    		ajaxOptions.success = function(msg){
	    			window.location.href = "${ctx}/report/operationchangesuccessrate/query.do?"+getQueryParam();
	    	    };
...
function selectAll(object){  
    if (object.checked) {  
    $("input[name='opercheckbox']").prop("checked", true);  
    } else {  
    $("input[name='opercheckbox']").prop("checked", false);  
    }  
    return false;
} 
...
function newreport(){
	window.location.href="${ctx}/report/operationchangesuccessrate/add_pre.do";
}
function refresh(){
	window.location.href="${ctx}/report/operationchangesuccessrate/query.do?"+getQueryParam();
}
function exportSystem(){
	window.location.href="${ctx}/report/operationchangesuccessrate/export.do?"+getQueryParam();
}
