...
$(function (){
$("#city_5").citySelect({
	url:"<%=request.getContextPath()%>/report/fakesiteattachmentrate/gettypes.do",
	prov:"${prov}",
	city:"${city}",
	dist:"${dist}",
	nodata:"none"
...
var url = '${ctx}/report/fakesiteattachmentrate/upload.do';
$('#fileupload').fileupload({
    ...
    add: function (e, data) { 
    	 ...
    	 window.location.href = "${ctx}/report/fakesiteattachmentrate/list.do?message="+data.result;
    }
...
<div class="container"> 
  ...
  <div id="content" class="row mt_78">
    ...
    <div class="col-md-10" id="right">
      <div class="breadcrumb"><span>您当前所在的位置：</span> <a>数据采集</a> &gt; <a>假冒网站查封率</a></div>
   <div class="row whitebackground mb_10">
        <div class="span12">
          <h3 class="show_tit">查询</h3>
          <div class="show_conment">
            <form class="form-horizontal" action="${ctx}/report/fakesiteattachmentrate/query.do?">
      ...
      <div class="row whitebackground mb_10">
        <div class="span12">
         ...
         <div class="show_tit3">
           <div id="operateBtns">
	<div class="pull-left">
	<h4 class="showtit4">假冒网站查封率列表</h4>
	</div>
<div class="pull-right">
		...
		<thead><tr><th><input type="checkbox" id="allfakeselected" onclick="selectAll(this)"/></th><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>假冒网站数量</th><th>已查封数量</th></tr></thead>
		<tbody>
		<c:choose>
			 	<c:when test="${!empty rates.result }">
		<c:forEach items="${rates.result}" var="rate">
			<tr>
			    <td><input type="checkbox" name="fakecheckbox" value="${rate.id}"/></td>
...
function deleteRates(){
	var deleteroleids = "";
	$("input[name='fakecheckbox']:checked").each(function(){     
	...
	if(""==deleteroleids){
		 ...
		 return;
	      ...
	      $.messager.confirm("系统提示", "您确定删除所选数据？", function() { 
	    	    ajaxOptions.url = "${ctx}/report/fakesiteattachmentrate/delete.do?";
	    		ajaxOptions.params = {"pageNo":pageNo,"deleteids":deleteroleids};
	    		ajaxOptions.success = function(msg){
	    			window.location.href="${ctx}/report/fakesiteattachmentrate/query.do?"+getQueryParam();
	    	    };
...
function selectAll(object){  
    if (object.checked) {  
    $("input[name='fakecheckbox']").prop("checked", true);  
    } else {  
    $("input[name='fakecheckbox']").prop("checked", false);  
    }  
    return false;
} 
...
function newreport(){
	window.location.href="${ctx}/report/fakesiteattachmentrate/add_pre.do";
}
function refresh(){
	window.location.href="${ctx}/report/fakesiteattachmentrate/query.do?"+getQueryParam();
}
function exportSystem(){
	window.location.href="${ctx}/report/fakesiteattachmentrate/export.do?"+getQueryParam();
}
