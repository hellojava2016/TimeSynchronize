...
<div class="container"> 
  ...
  <div id="content" class="row mt_78">
    ...
    <div class="col-md-10" id="right">
      <div class="breadcrumb"><span>您当前所在的位置：</span> <a>风险分析</a> &gt; <a>系统交易成功率</a></div>
   <div class="row whitebackground mb_10">
               ...
               <div class="col-xs-12 row">      
                 <label   class="col-xs-1 control-label pr_0 wid_10">区域：</label>        
				  ...
				  <label   class="col-xs-1 control-label pr_0 wid_10">风险指标：</label>
                   <div id="city_5">
  		              <select class="prov wid_2" name="prov" id="prov" style="width: 160px;"></select>
  		              <select class="city wid_2" name="city" id="city" style="width: 140px;" disabled="disabled"></select>
  		              <select class="dist wid_2" name="dist" id="dist" disabled="disabled"></select>
                   </div>
              </div>    
               ...
               <div class="col-xs-12 row dpheight15"></div>
                 ...
                 <div class="col-xs-3 pr_0 pl_0 mr_10">  
                  <select name="reportorder"  class="selectcsss" id="reportorder">                      
                      <option value="4" <c:if test="${reportorder==4}">selected</c:if>>交易成功率-降序</option> 
                      <option value="5" <c:if test="${reportorder==5}">selected</c:if>>交易成功率-升序</option>                 
	...
	<div class="pull-right">
		...
		<thead><tr><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>交易总量</th><th>交易成功量</th><th>交易成功率(%)</th></tr></thead>
		<tbody>
		<c:choose>
		...
		<c:forEach items="${rates.result}" var="rate">
			<tr>
				...
				<td>${rate.aost}</td>	
				<td>${rate.aosst}</td>
				<td><fmt:formatNumber  value="${rate.percent}"   type="number" pattern="0.00" /></td>			
			</tr>
		          </c:forEach>
	       	     </c:when>
						<c:otherwise>
		    ...
		    <tr style="background-color:#eee">
					<th colspan="8"><tags:pagination page="${rates}" paginationSize="7"/></th>
			</tr>
		</tbody>
	</table>
...
function refresh(){
	window.location.href="${ctx}/report/systemtransactionsuccessrate/report.do?"+getQueryParam();
}
</script>
</body>
</html>
