...
$(function (){
	$("#city_5").citySelect({
		url:"<%=request.getContextPath()%>/report/electronictransactionchangerate/gettypes.do",
		prov:"${prov}",
		city:"${city}",
		dist:"${dist}",
		nodata:"none"
	...
	var url = '${ctx}/report/electronictransactionchangerate/upload.do';
	$('#fileupload').fileupload({
	    ...
	    add: function (e, data) { 
	    	 ...
	    	 window.location.href = "${ctx}/report/electronictransactionchangerate/list.do?message="+data.result;
	    }
...
<div class="container"> 
  ...
  <div id="content" class="row mt_78">
    ...
    <div class="col-md-10" id="right">
      <div class="breadcrumb"><span>您当前所在的位置：</span> > <a>数据采集</a> &gt; <a>主要电子渠道交易变化率</a></div>
      
     <div class="row whitebackground mb_10">
        <div class="span12">
          <h3 class="show_tit">查询</h3>
          <div class="show_conment">
            <form class="form-horizontal" action="${ctx}/report/electronictransactionchangerate/query.do?">
      ...
      <div>
        ...
        <div class="span12">
          ...
          <div class="show_tit3">
           <div id="operateBtns">
	<div class="pull-left">
	<h4 class="showtit4">主要电子渠道交易变化率列表</h4>
	</div>
	<div class="pull-right">
		...
		<thead><tr><th><input type="checkbox" id="allroleselected" onclick="selectAll(this)"/></th><th>所属机构</th><th>指标类型</th><th>指标日期(期数)</th><th>是否补报</th><th>当前交易量</th></tr></thead>
		<tbody>
		<c:choose>
		...
		<c:forEach items="${rates.result}" var="rate">
			<tr>
				...
				<td>${rate.currentTradingVolume}</td>
			</tr>
	 </c:forEach>
	       	     </c:when>
						<c:otherwise>
...
function deleteRates(){
	...
	$.messager.model = { 
	      ...
	      $.messager.confirm("系统提示", "您确定删除所选数据？", function() { 
	    	    ajaxOptions.url = "${ctx}/report/electronictransactionchangerate/delete.do?";
	    		ajaxOptions.params = {"pageNo":pageNo,"deleteids":deleteroleids};
	    		ajaxOptions.success = function(msg){
	    			window.location.href = "${ctx}/report/electronictransactionchangerate/query.do?"+getQueryParam();
	    	    };
...
function newreport(){
	window.location.href="${ctx}/report/electronictransactionchangerate/add_pre.do";
}
function refresh(){
	window.location.href="${ctx}/report/electronictransactionchangerate/query.do?"+getQueryParam();
}
function exportSystem(){
	window.location.href="${ctx}/report/electronictransactionchangerate/export.do?"+getQueryParam();
}
